...
router.get('/login', passwordController.getLogin, (req, res) => {
  return res.status(200).json({userMetaData: res.locals.userMetaData}
  );
});
...
router.post(
  ...
  (req, res) => {
    return res.status(200).json({
        userMetaData: res.locals.userMetaData
      },
    );
  }
);
...
router.get(
  '/badUserPasswordCheck',
  passwordController.getBadPasswordCheck,
  (req, res) => {
    return res
      .status(200)
      .json({ isBadPasswordUser: res.locals.isBadPasswordUser });
  }
);
